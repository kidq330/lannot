...
type annotation = int * string * stmt * location * exp * exp * stmt * stmt
...
module type ANNOTATOR_Strong = sig
  val name : string
  val help : string
  val compute : (extra:string list -> Cil_types.stmt -> Cil_types.exp -> Cil_types.exp -> Cil_types.stmt -> Cil_types.stmt -> Cil_types.exp -> Cil_types.location -> Cil_types.stmt) -> Cil_types.file -> unit
end
...
let strongmutation names ast =
  let fa acc name =
    try
      let acc = ref acc in
      let annotator = (Hashtbl.find annotators name) in
      Options.feedback "apply annotations for %s@." annotator.name;
      annotator.compute nextId acc ast;
...
let mk_compute compute name nextId acc ast =
  let label_maker cond loc =
    ...
    let id = !nextId in
    incr nextId;
    acc := (id,tag,Cil.mkEmptyStmt(),loc,cond,cond,Cil.mkEmptyStmt(),Cil.mkEmptyStmt()) :: !acc;
...
let mk_compute_extras compute name nextId acc ast =
  let label_maker ~extra label loc =
    let tag = String.concat " " (name::extra) in
    let id = !nextId in
    incr nextId;
    acc := (id,tag,Cil.mkEmptyStmt(),loc,label,label,Cil.mkEmptyStmt(),Cil.mkEmptyStmt()) :: !acc;
    let tagExp = Utils.mk_exp (Const (CStr tag)) in
    let idExp = Utils.mk_exp (Const (CInt64 (Integer.of_int id, IInt, None))) in
    Utils.mk_call "pc_label" [ label; idExp; tagExp ]
  in
  compute label_maker ast

let mk_compute_Strong compute name nextId acc ast =
  let label_makers ~extra stmt orig mut origstmt mutstmt labl loc =
    let tag = String.concat " " (name::extra) in
    let id = !nextId in
    incr nextId;
    acc := (id,tag,stmt,loc,orig,mut,origstmt,mutstmt) :: !acc;
    let tagExp = Utils.mk_exp (Const (CStr tag)) in
    let idExp = Utils.mk_exp (Const (CInt64 (Integer.of_int id, IInt, None))) in
    Utils.mk_call "pc_label" [ labl; idExp; tagExp ]
  in
  compute label_makers ast
...
module RegisterStrong (A : ANNOTATOR_Strong) = struct
  let self = { name = A.name; help = A.help; compute = mk_compute_Strong A.compute A.name }
  let () = register_annotator self
end
...
let shouldInstrument fun_varinfo =
  ...
  if (*fun_varinfo.vname = "main" ||*) fun_varinfo.vname = "pc_label" || fun_varinfo.vname = "covlabels_init" || fun_varinfo.vname = "covlabels_exit" then
    false
  else if Datatype.String.Set.is_empty names then
    true
  else
